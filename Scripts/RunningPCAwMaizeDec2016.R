...
#Meant for making plots with maize

setwd("/Users/davehuf/Hufford Lab/16/9_ReReRedoPCA/1_RedoneWNewColorsDec2016")
data = read.table("ZeaAllInfo.14col.OrigForm.PCAmatrix.wMz", row.names=1, skip=1) #SNP file used to subset
...
for(i in seq(1:nrow(info))){
	...
	else if(spec[i]=="Zea_mays_mays" && !is.na(spec[i]) && cntry[i]=="Mexico"){
		cc = "gray38"
		sh = 1
	}
	else{
...
# Plot results
pdf("PC1vPC2wMz.pdf")
plot(newpc$x[,2]~newpc$x[,1],col=data3[,ncol(data3)], pch=as.numeric(data3[,ncol(data3)-1]) ,ylab="PC 2   (4.77% var. explained)",xlab="PC 1   (7.83% var. explained)")
dev.off()

pdf("PC3vPC4wMz.pdf")
plot(newpc$x[,4]~newpc$x[,3],col=data3[,ncol(data3)], pch=as.numeric(data3[,ncol(data3)-1]) ,ylab="PC 4   (1.06% var. explained)",xlab="PC 3   (1.62% var. explained)")
dev.off()

pdf("PC5vPC6wMz.pdf")
plot(newpc$x[,6]~newpc$x[,5],col=data3[,ncol(data3)], pch=as.numeric(data3[,ncol(data3)-1]) ,ylab="PC 6   (0.82% var. explained)",xlab="PC 5   (1.04% var. explained)")
dev.off()

pdf("PC7vPC8wMz.pdf")
plot(newpc$x[,8]~newpc$x[,7],col=data3[,ncol(data3)], pch=as.numeric(data3[,ncol(data3)-1]) ,ylab="PC 8   (0.71% var. explained)",xlab="PC 7   (0.77% var. explained)")
dev.off()

pdf("PC9vPC10wMz.pdf")
plot(newpc$x[,10]~newpc$x[,9],col=data3[,ncol(data3)], pch=as.numeric(data3[,ncol(data3)-1]) ,ylab="PC 10   (0.58% var. explained)",xlab="PC 9   (0.66% var. explained)")
dev.off()
